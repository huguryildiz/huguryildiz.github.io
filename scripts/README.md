...
| --- | --- | --- | --- |
| `fetch_scholar.py` | `.github/workflows/update-scholar.yml` — monthly, day 1 at 01:30 UTC, plus manual dispatch | `_data/scholar_metrics.json` | `SERPAPI_KEY`, `SCHOLAR_AUTHOR_ID` (repo secrets); Python `requests` |
...
Both workflows commit only when the snapshot actually changed, and tag the commit
`[skip ci]` so the refresh does not trigger a redeploy.
