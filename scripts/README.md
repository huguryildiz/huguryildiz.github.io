...
gap unavailable instead of presenting a partial result as exact. Content-page totals and enabled
screen-size totals must reconcile with the temporal page-view block. Country, browser, system,
and language totals need not: GoatCounter can record an empty value for each, and the public
rankings exclude those unknown values rather than inferring them. What each run collects,
beyond the preset blocks and
